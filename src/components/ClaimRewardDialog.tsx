      ...
      <DialogContent className="max-w-sm bg-gradient-to-br from-background via-background to-primary/10 border-primary/30">
        <DialogHeader>
          <DialogTitle className="text-center text-xl font-heading">
            {claimed ? "🎉 Selamat!" : "Claim Income Harian"}
          </DialogTitle>
        </DialogHeader>
              ...
              {[...Array(20)].map((_, i) => (
                ...
                >
                  <Sparkles 
                    className="w-4 h-4" 
                    style={{ 
                      color: ['#00F5FF', '#FF00E5', '#FFD700'][Math.floor(Math.random() * 3)] 
                    }} 
                  />
                </div>
              ))}
        ...
        {!claimed && (
          <Button
            onClick={handleClaim}
            className="w-full bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-primary-foreground font-semibold h-12 shadow-lg shadow-primary/30"
          >
        ...
        {claimed && (
          <div className="flex items-center justify-center gap-2 text-success animate-fade-in">
            ...
            <span className="font-medium">Income berhasil di-claim!</span>
            <Sparkles className="w-4 h-4" />
          </div>
        )}
      </DialogContent>
